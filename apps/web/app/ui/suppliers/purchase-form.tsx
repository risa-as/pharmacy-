...
export default function PurchaseForm({
  ...
  const updateItem = (drugId: string, field: keyof LineItem, value: any) => {
    setItems((prev) =>
      prev.map((i) => (i.drugId === drugId ? { ...i, [field]: value } : i)),
    );
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (hasZeroCost) {
      setError("يجب إدخال سعر التكلفة لجميع الأصناف");
      return;
    }
    setError("");
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-muted/50 text-right text-xs font-semibold text-muted-foreground border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-2.5 w-10">الإجراءات</th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <input
                        ...
                        onChange={(e) =>
                          updateItem(item.drugId, "expiryDate", e.target.value)
                        }
                    ...
                    <td className="px-4 py-2.5">
                      <input
                        ...
                        onChange={(e) =>
                          updateItem(item.drugId, "batchNumber", e.target.value)
                        }
                        placeholder="اختياري"
                    ...
                    <td
                      className="px-4 py-2.5 text-left font-bold text-foreground tabular-nums"
                      dir="ltr"
                    >
      ...
      {items.length === 0 && (
        <div className="rounded-xl border-2 border-dashed border-border py-14 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            ابحث وأضف أدوية للفاتورة من الحقل أعلاه
          </p>
        </div>
      )}
