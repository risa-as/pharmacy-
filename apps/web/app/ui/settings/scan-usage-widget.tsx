...
export default function ScanUsageWidget() {
  ...
  const color = pct >= 90 ? 'bg-destructive' : pct >= 70 ? 'bg-warning' : 'bg-emerald-500';
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-bold text-foreground">{data.used} / {data.limit} مسح</span>
          </div>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>المتبقي: <span className="font-bold text-foreground">{data.remaining} مسح</span></span>
