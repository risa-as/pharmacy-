...
export default function Form() {
  ...
  const [state, dispatch] = useActionState(createDrug, initialState);
  ...
  return (
    <form action={dispatch}>
      <div className="rounded-md bg-muted p-4 md:p-6">
        ...
        <div className="mb-4 flex items-center gap-3 mt-6 border-t border-border pt-4">
          <input
            id="isActive"
            name="isActive"
            type="checkbox"
            defaultChecked={true}
            className="h-5 w-5 rounded border-border bg-background text-primary focus:ring-2 focus:ring-primary/30"
          />
          <label
            htmlFor="isActive"
            className="text-sm font-medium cursor-pointer"
          >
            تفعيل الدواء (نشط ومتاح للبيع)
          </label>
        </div>
      </div>
