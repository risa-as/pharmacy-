...
export default async function StocktakesTable({
  ...
  const sessionBranchId = ctx.user.branchId;
  ...
  const stocktakes = await prisma.stocktake.findMany({
    where: {
      AND: [ctx.tenantBranchWhere, { branchId: effectiveBranchId }],
    },
    include: {
  ...
  return (
    <div className="mt-6 flow-root">
      {stocktakes.some((s) => s.status === "REVIEW") && (
        <div className="mb-4 rounded-lg border border-warning/30 bg-warning/10 p-4">
          <p className="font-bold">
            {stocktakes.filter((s) => s.status === "REVIEW").length} عمليات جرد
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {stocktakes
              ...
              .map((s) => (
                <Link
                  key={s.id}
                  href={`/dashboard/inventory/stocktakes/${s.id}`}
                  className="rounded-lg border border-border px-3 py-2"
                >
                  مراجعة العجز · {s.documentNumber}
                </Link>
              ))}
          </div>
        </div>
                  ...
                  <td className="whitespace-nowrap py-3 pl-6 pr-3">
                    <div className="flex items-center gap-3">
                      <p className="font-medium text-foreground">
                        {stocktake.documentNumber}
                      </p>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    {new Date(stocktake.createdAt).toLocaleDateString("ar-IQ", {
                      timeZone: "Asia/Baghdad",
                    })}{" "}
                    {new Date(stocktake.createdAt).toLocaleTimeString("ar-IQ", {
                      timeZone: "Asia/Baghdad",
                    })}
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
