...
export async function receivePurchaseStock(
  ...
  if (!Array.isArray(input) || input.length === 0 || input.length > 500) {
    throw new PurchaseReceiptError(
      "أرسل أصناف الاستلام كاملة (بحد أقصى 500 بند).",
    );
  }
  return db.$transaction(
    async (tx) => {
      ...
      if (!purchase)
        throw new PurchaseReceiptError("طلب الشراء غير موجود ضمن نطاقك.", 404);
      const linkedEvent = await tx.warehouseOrderEvent.findFirst({
        where: {
          type: "APPROVED",
          payload: { path: ["purchaseId"], equals: purchaseId },
        },
        select: { orderId: true },
      });
      const linkedId = purchase.warehouseOrderId || linkedEvent?.orderId;
      ...
      if (linkedOrder && !["SHIPPED", "DELIVERED"].includes(linkedOrder.status))
        throw new PurchaseReceiptError(
          "طلب المذخر لم يخرج للتسليم بعد؛ لا يمكن استلامه.",
        );
      ...
      const lines = input.map((raw: ReceiptInput) => {
        ...
        if (!item)
          throw new PurchaseReceiptError("الصنف لا ينتمي لفاتورة الشراء.");
        ...
        if (
          ...
          throw new PurchaseReceiptError(
            "الاستلام الكامل يتطلب الكمية المعتمدة لكل بند؛ لا يمكن إغلاق فاتورة بكميات مختلفة.",
          );
        }
        if (typeof raw.batchNumber !== "string" || !raw.batchNumber.trim())
          throw new PurchaseReceiptError("رقم الدفعة مطلوب.");
        if (
          !(raw.expiryDate instanceof Date) &&
          typeof raw.expiryDate !== "string"
        )
          throw new PurchaseReceiptError("تاريخ الانتهاء غير صالح.");
        const expiry = new Date(raw.expiryDate);
        if (!Number.isFinite(expiry.getTime()) || expiry <= new Date())
          throw new PurchaseReceiptError(
            "يجب أن يكون تاريخ الانتهاء صحيحاً وفي المستقبل.",
          );
        let cost = item.cost;
        if (raw.cost !== undefined && raw.cost !== null) {
          if (
            ...
            raw.cost < 0 ||
            raw.cost > MAX_UNIT_COST
          ) {
            throw new PurchaseReceiptError("سعر الشراء غير صالح.");
          }
          cost = raw.cost;
        }
        if (linkedOrder && cost !== item.cost)
          throw new PurchaseReceiptError(
            "سعر وبونص طلب المذخر معتمدان؛ لا تعدلهما أثناء الاستلام.",
          );
        if (cost === 0 && !(linkedOrder && item.cost === 0)) throw new PurchaseReceiptError('أدخل تكلفة الشراء قبل الاستلام؛ الصفر مسموح فقط للبند المجاني المعتمد من المذخر.');
        const key = `${item.id}:${raw.batchNumber.trim()}:${expiry.toISOString()}`;
        if (seen.has(key))
          throw new PurchaseReceiptError("الدفعة مكررة لنفس بند الشراء.");
        seen.add(key);
        const prior = receivedByItem.get(item.id);
        if (prior && prior.cost !== cost)
          throw new PurchaseReceiptError(
            "يجب أن يتطابق سعر الوحدة في دفعات بند الشراء الواحد.",
          );
        receivedByItem.set(item.id, {
          quantity: (prior?.quantity ?? 0) + raw.quantity,
          cost,
        });
        return {
          item,
      ...
      if (
        receivedByItem.size !== purchase.items.length ||
        purchase.items.some(
          (item) => receivedByItem.get(item.id)?.quantity !== item.quantity,
        )
      )
        throw new PurchaseReceiptError(
          "يجب استلام جميع بنود الفاتورة قبل إغلاقها.",
        );
      ...
      const total = Math.max(
        ...
        purchase.total +
          lines.reduce(
            (sum, l) => sum + l.quantity * (l.cost - l.item.cost),
            0,
          ),
      );
      ...
      const updated = await tx.purchase.update({
        where: { id: purchaseId },
        data: { status: "COMPLETED", paidAmount, total },
      });
      if (actor)
