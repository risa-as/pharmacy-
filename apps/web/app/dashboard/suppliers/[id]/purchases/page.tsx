...
export default async function SupplierPurchasesPage({
  ...
  const supplier = await prisma.supplier.findUnique({
    where: {
      id: params.id,
      organizationId: tenantCtx.organizationId || undefined,
    },
        ...
        <table className="min-w-full text-foreground">
          <thead className="bg-muted text-right text-sm font-semibold text-muted-foreground border-b border-border">
            <tr>
              ...
              <th className="px-6 py-4 font-cairo">الإجراءات</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {purchases.map((p: any, idx: number) => (
              <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td
                  className="px-6 py-4 font-bold text-right text-foreground tabular-nums"
                  dir="ltr"
                >
