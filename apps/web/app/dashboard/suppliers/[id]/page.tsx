        ...
        <div className="w-full" dir="rtl">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <Link href="/dashboard/suppliers" className="p-2 rounded-lg hover:bg-muted transition-colors">
                    ...
                    <div>
                        ...
                        {supplier.phone && (
                            <p className="text-sm text-muted-foreground mt-1" dir="ltr">{supplier.phone}</p>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-cairo text-muted-foreground">إجمالي المشتريات</span>
                    </div>
                    <p className="text-2xl font-bold text-foreground">
                        {totalPurchased.toLocaleString('en-US')} <span className="text-sm text-muted-foreground">د.ع</span>
                    </p>
                </div>
                ...
                <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-cairo text-muted-foreground">إجمالي المدفوع</span>
                    </div>
                    <p className="text-2xl font-bold text-foreground">
                        {totalPayments.toLocaleString('en-US')} <span className="text-sm text-muted-foreground">د.ع</span>
                    </p>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {ledger.map((entry) => (
                            <tr key={entry.id} className="hover:bg-muted transition-colors">
                                ...
                                <td className="px-6 py-3 text-sm text-foreground">{entry.description}</td>
                                <td className="px-6 py-3 text-sm text-muted-foreground">{entry.branch}</td>
                                <td className="px-6 py-3 text-sm font-bold text-destructive whitespace-nowrap">
                                    {entry.type === 'purchase' ? entry.amount.toLocaleString('en-US') : '—'}
                                </td>
