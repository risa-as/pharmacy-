...
function priceJump(item: ReceiveItem): boolean {
  ...
  return (
    cost / item.lastCost >= PRICE_JUMP_RATIO ||
    item.lastCost / cost >= PRICE_JUMP_RATIO
  );
}
...
function itemErrors(item: ReceiveItem, nowYear: number) {
  ...
  const cost = Number(item.unitCost);
  return {
    cost: !item.unitCost.trim()
      ? "أدخل سعر الشراء"
      : !Number.isFinite(cost) || cost < 0 || (cost === 0 && !item.approvedFree)
        ? "سعر غير صالح"
        : null,
    quantity: !item.receivedQuantity.trim()
...
export default function ReceiveItemsScreen() {
    ...
    try {
      ...
      setItems(
        data.items.map((item: any) => ({
          ...item,
          itemId: item.id,
          approvedFree: Boolean(data.warehouseOrderId && item.cost === 0),
          ...
          batchNumber:
            item.shippedPrefill?.kind === "SINGLE"
              ? item.shippedPrefill.batchNumber
              : (item.batchNumber ?? ""),
          exactExpiry:
            item.shippedPrefill?.kind === "SINGLE"
              ? item.shippedPrefill.expiryDate
              : undefined,
          expiryMonth:
            item.shippedPrefill?.kind === "SINGLE"
              ? String(new Date(item.shippedPrefill.expiryDate).getMonth() + 1)
              : "",
          expiryYear:
            item.shippedPrefill?.kind === "SINGLE"
              ? String(new Date(item.shippedPrefill.expiryDate).getFullYear())
              : "",
          lastCost:
            typeof item.lastCost === "number" && item.lastCost > 0
              ? item.lastCost
              : null,
  ...
  const updateItem = (
    ...
    setItems((prev) =>
      prev.map((it, i) =>
        i === index ||
        (field === "unitCost" && it.itemId === prev[index].itemId)
          ? {
              ...it,
              [field]: value,
              ...(["expiryMonth", "expiryYear"].includes(field)
                ? { exactExpiry: undefined }
                : {}),
            }
          : it,
      ),
  ...
  const firstInvalid = allErrors.findIndex((e) =>
    ...
    try {
      const payload = items.map((item) => ({
        ...
        expiryDate:
          ...
          new Date(
            Date.UTC(
              Number(item.expiryYear),
              Number(item.expiryMonth),
              0,
              23,
              59,
              59,
  ...
  const handleConfirm = () => {
    ...
    if (firstInvalid >= 0) {
      Alert.alert(
        "بيانات ناقصة",
        `أكمل بيانات "${items[firstInvalid].drugName}" قبل الاستلام.`,
      );
      return;
    }
    const mismatched = items.some(
      (i) =>
        items
          .filter((other) => other.itemId === i.itemId)
          .reduce((sum, other) => sum + Number(other.receivedQuantity), 0) !==
        i.quantity,
    );
    if (mismatched) {
      Alert.alert(
        "استلام كامل فقط",
        "يجب مطابقة الكميات المعتمدة؛ لا يمكن إغلاق الطلب باستلام جزئي.",
      );
      return;
    }
    const differs = items.filter(
      (i) => parseInt(i.receivedQuantity, 10) !== i.quantity,
    );
    const lines = items
      .map((i) => {
    ...
    const warnings = [
      ...
      jumps.length
        ? `سعر ${jumps.map((i) => `«${i.drugName}»`).join("، ")} يختلف كثيراً عن آخر سعر شراء — تأكد أنه سعر الوحدة (الشريط) وليس الباكيت.`
        : "",
    ].filter(Boolean);
    Alert.alert(
      "تأكيد الاستلام",
  ...
  const header = (
    <ScreenHeader title="استلام المشتريات" fallbackHref="/(tabs)/purchases" />
  );
    ...
    >
      ...
      >
        ...
        {items.map((item, index) => {
          ...
          const differs =
            !isNaN(received) && received > 0 && received !== item.quantity;
          return (
            <Surface key={item.id} style={{ gap: 12 }}>
              ...
              {item.shippedPrefill?.kind === "MULTIPLE" && (
                <InfoNote
                  tone="warning"
                  title="شحن من عدة دفعات"
                  text={`قسّم الاستلام بحسب الدفعات الفعلية. الدفعات المشحونة (الكميات بالباكيت): ${item.shippedPrefill.batches?.map((b) => `${b.batchNumber} · ${b.quantity} · ${String(b.expiryDate).slice(0, 10)}`).join(" / ")}`}
                />
              )}
              <AppButton
                label="تقسيم الكمية على دفعة أخرى"
                compact
                variant="outline"
                disabled={submitting}
                onPress={() =>
              ...
              <FormField
                ...
                hint={
                  differs
                    ? `تختلف عن الكمية المطلوبة (${formatNumber(item.quantity)})`
                    : undefined
                }
              />
              <FormField
                ...
                hint={
                  jump
                    ? `يختلف كثيراً عن آخر سعر (${formatNumber(item.lastCost ?? 0)} د.ع) — تأكد أنه سعر الشريط`
                    : item.unitCost.trim() && costValue === 0
                      ? "صفر = بونص مجاني من المورد"
                      : item.lastCost
                        ? `آخر سعر شراء: ${formatNumber(item.lastCost)} د.ع`
              ...
              <View style={{ flexDirection: "row-reverse", gap: 10 }}>
                <FormField
                  ...
                  onChangeText={(v) =>
                    updateItem(index, "expiryMonth", v.replace(/[^0-9]/g, ""))
                  }
                  error={e.month}
                />
                <FormField
                  ...
                  onChangeText={(v) =>
                    updateItem(index, "expiryYear", v.replace(/[^0-9]/g, ""))
                  }
                  onBlur={() =>
                    updateItem(
                      index,
                      "expiryYear",
                      normalizeYear(item.expiryYear),
                    )
                  }
                  error={e.year}
                />
        ...
        <Surface style={{ gap: 8 }}>
          ...
          >
            يرجى التأكد من صحة البيانات قبل تأكيد الاستلام
          </Text>
          {items.map((item, i) => (
            <View
              key={item.id}
              ...
              <SummaryLine
                ...
                value={
                  item.receivedQuantity
                    ? formatNumber(parseInt(item.receivedQuantity, 10))
                    : "—"
                }
              />
              <SummaryLine
                ...
                value={
                  item.unitCost.trim()
                    ? Number(item.unitCost) > 0
                      ? `${formatNumber(Number(item.unitCost))} د.ع`
                      : "مجاني"
                    : "—"
                }
              ...
              <SummaryLine
                ...
                value={
                  item.expiryMonth && item.expiryYear
                    ? `${item.expiryMonth.padStart(2, "0")}/${item.expiryYear}`
                    : "—"
                }
          ...
          >
            ...
            <Text
              style={{ color: C.foreground, fontSize: 18, fontWeight: "900" }}
            >
